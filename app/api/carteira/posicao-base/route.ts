...
/**
 * GET /api/carteira/posicao-base
 * Retorna a posição base do usuário (data_base, itens, prejuízo IR acumulado).
 */
export async function GET() {
  ...
  const itens  = await sql`SELECT ticker, quantidade::float, preco_medio::float FROM posicao_base_itens WHERE user_id = ${userId} ORDER BY ticker`
 ...
 * Salva (ou substitui) a posição base do usuário e reconstrói a carteira.
 *
 * Body: {
 *   data_base: "YYYY-MM-DD",
 *   itens: [{ ticker, quantidade, preco_medio }],
 *   prejuizo_swing?: number,   // R$ — prejuízo acumulado swing trade
 *   prejuizo_day?:   number,   // R$ — prejuízo acumulado day trade
 * }
 */
export async function POST(req: NextRequest) {
  ...
  const itens: { ticker: string; quantidade: number; preco_medio: number }[] = body.itens ?? []
  ...
  // Substitui todos os itens da posição base
  ...
  for (const item of itens) {
    ...
    await sql`
      INSERT INTO posicao_base_itens (user_id, ticker, quantidade, preco_medio)
      VALUES (${userId}, ${t}, ${qt}, ${pm})
    `
  }

  // Salva prejuízo acumulado de IR
  ...
  // Reconstrói a carteira a partir desta nova base
