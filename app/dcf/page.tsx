...
function SecKPIs({ e, precoLive }: { e: any; precoLive: number|null }) {
  ...
  if (kl.sell_out != null)     kpis.push({ label: 'Sell-Out',           val: `R$ ${f1(kl.sell_out)}M` })
  if (kl.sell_out_yoy != null) kpis.push({ label: 'Sell-Out YoY',       val: `${kl.sell_out_yoy > 0 ? '+' : ''}${f1(kl.sell_out_yoy)}%` })
  if (kl.sell_in != null)      kpis.push({ label: 'Sell-In',            val: `R$ ${f1(kl.sell_in)}M` })
  if (kl.sell_in_yoy != null)  kpis.push({ label: 'Sell-In YoY',        val: `${kl.sell_in_yoy > 0 ? '+' : ''}${f1(kl.sell_in_yoy)}%` })
  if (kl.rec_sellout_lp != null) kpis.push({ label: 'LP Sell-Out',      val: `R$ ${f1(kl.rec_sellout_lp)}M` })
  if (kl.rec_ecom != null)     kpis.push({ label: 'E-commerce',         val: `R$ ${f1(kl.rec_ecom)}M` })
  if (kl.rec_franquia != null) kpis.push({ label: 'Franquias',          val: `R$ ${f1(kl.rec_franquia)}M` })
  if (kl.rec_multimarca != null) kpis.push({ label: 'Multimarcas',      val: `R$ ${f1(kl.rec_multimarca)}M` })
...
function SecProximoTri({ e, plano, localEst, setLocalEst }: { e: any; plano: string | null; localEst: Record<string, any> | null; setLocalEst: (v: Record<string, any> | null) => void }) {
  ...
  if (est.sell_out   != null || kl.sell_out   != null) compRows.push({ label:'Sell-Out (R$ MM)',  est: est.sell_out,   real: kl.sell_out   })
  if (est.sell_in    != null || kl.sell_in    != null) compRows.push({ label:'Sell-In (R$ MM)',   est: est.sell_in,    real: kl.sell_in    })
  if (est.sell_out_yoy != null || kl.sell_out_yoy != null) compRows.push({ label:'Sell-Out YoY (%)', est: est.sell_out_yoy, real: kl.sell_out_yoy })
  if (est.sell_in_yoy  != null || kl.sell_in_yoy  != null) compRows.push({ label:'Sell-In YoY (%)',  est: est.sell_in_yoy,  real: kl.sell_in_yoy  })
