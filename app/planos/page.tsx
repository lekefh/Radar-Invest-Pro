...
const PLANOS = [
  ...
  },
  {
    ...
    recursos: [
      ...
      'Suporte prioritário WhatsApp',
      'Acesso antecipado a novas empresas',
      'Atualizações de análise em primeira mão',
    ],
    bloqueados: [],
  },
]
...
export default function PlanosPage() {
  ...
  const desconto = Math.round((1 - (499 / (49.90 * 12))) * 100)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050d1a', color: '#e8edf4', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ textAlign: 'center', padding: '72px 24px 48px' }}>
        ...
        <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 'clamp(36px,5vw,56px)', fontWeight: 700, letterSpacing: -1.5, marginBottom: 16, lineHeight: 1.1 }}>
          Análise como gestor de fundo.<br />
          <span style={{ color: '#e8a020' }}>Ao preço de investidor individual.</span>
        </h1>
        <p style={{ color: '#6b84a8', fontSize: 17, maxWidth: 520, margin: '0 auto 40px', lineHeight: 1.6 }}>
          Ferramenta profissional para quem leva a sério a carteira de ações.
        </p>

        {/* Toggle mensal/anual */}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 100, padding: '4px 6px' }}>
          <button onClick={() => setAnual(false)} style={{ padding: '7px 20px', borderRadius: 100, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, background: !anual ? '#fff' : 'transparent', color: !anual ? '#050d1a' : '#6b84a8', transition: 'all .2s' }}>
            Mensal
          </button>
          <button onClick={() => setAnual(true)} style={{ padding: '7px 20px', borderRadius: 100, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, background: anual ? '#e8a020' : 'transparent', color: anual ? '#000' : '#6b84a8', transition: 'all .2s', display: 'flex', alignItems: 'center', gap: 8 }}>
            Anual
      ...
      {/* Cards */}
      <div style={{ display: 'flex', gap: 24, justifyContent: 'center', padding: '0 24px 80px', flexWrap: 'wrap', maxWidth: 1100, margin: '0 auto' }}>
        {PLANOS.map(p => {
          ...
          return (
            <div key={p.id} style={{
              ...
              background: p.destaque ? 'linear-gradient(135deg,rgba(102,187,106,.06),rgba(232,160,32,.04))' : 'rgba(255,255,255,.03)',
              border: `1.5px solid ${p.destaque ? 'rgba(102,187,106,.4)' : ativo ? 'rgba(232,160,32,.4)' : 'rgba(255,255,255,.07)'}`,
              borderRadius: 16, padding: '32px 28px', position: 'relative', transition: 'transform .2s',
            }}>
              {p.destaque && (
              ...
              {ativo && (
                <div style={{ position: 'absolute', top: -13, right: 20, background: '#e8a020', color: '#000', borderRadius: 100, padding: '3px 14px', fontSize: 11, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase' }}>
                  Seu plano
                </div>
              )}

              <div style={{ marginBottom: 8, fontSize: 12, fontWeight: 700, color: p.cor, letterSpacing: 1.5, textTransform: 'uppercase' }}>{p.nome}</div>

              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, marginBottom: 4 }}>
                {p.preco === 0
                  ? <span style={{ fontSize: 42, fontWeight: 800, fontFamily: 'Space Grotesk, sans-serif', color: '#e8edf4' }}>Grátis</span>
                  : <>
                      <span style={{ fontSize: 14, color: '#6b84a8', marginBottom: 8 }}>R$</span>
                      <span style={{ fontSize: 46, fontWeight: 800, fontFamily: 'Space Grotesk, sans-serif', lineHeight: 1, color: '#e8edf4' }}>
                        {preco.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </span>
                      <span style={{ fontSize: 13, color: '#6b84a8', marginBottom: 8 }}>{sufixo}</span>
                    </>
                }
              </div>
              {anual && p.preco > 0 && (
                <div style={{ fontSize: 12, color: '#6b84a8', marginBottom: 20 }}>
                  equivale a R$ {(preco / 12).toFixed(2).replace('.', ',')}/mês
                </div>
              )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
                {p.recursos.map(r => (
                  <div key={r} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 13.5, color: '#b8c4d4' }}>
                    <span style={{ color: '#66BB6A', fontSize: 15, marginTop: 1, flexShrink: 0 }}>✓</span>
                    {r}
                  </div>
                ))}
                {p.bloqueados.map(r => (
                  <div key={r} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 13.5, color: 'rgba(255,255,255,.2)' }}>
              ...
              {p.id === 'gratuito'
                ? (
                  <Link href={logado ? '/dashboard' : '/cadastro'} style={{
                    display: 'block', textAlign: 'center', padding: '13px', borderRadius: 8, fontSize: 14, fontWeight: 700,
                    background: 'rgba(255,255,255,.06)', color: '#e8edf4', textDecoration: 'none', border: '1px solid rgba(255,255,255,.1)',
                  }}>
                ...
                : (
                  <button
                    ...
                    style={{
                      width: '100%', padding: '13px', borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: ativo ? 'default' : 'pointer',
                      background: ativo ? 'rgba(232,160,32,.15)' : p.destaque ? '#66BB6A' : '#e8a020',
      ...
      {/* Garantia */}
      <div style={{ textAlign: 'center', padding: '0 24px 80px', color: '#6b84a8', fontSize: 13 }}>
